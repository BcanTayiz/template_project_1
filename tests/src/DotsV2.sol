...
contract DotsV2 is ReentrancyGuard, Ownable {

//mappings
mapping (uint => userData) public idFull;
mapping (uint => uint) public countryCount;
...
function Transaction( uint  _idFlag,  uint _exCountry, uint _country) external payable  {
    ...
    require(msg.value > idFull[_idFlag].balanceIdFlag,"can't lowball");
...
function GetSingle(uint _id) public view returns(userData memory)
{
    return idFull[_id];
}
//GetItems from maps
